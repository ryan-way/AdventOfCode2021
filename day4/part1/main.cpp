...
bool checkBoardWon(std::vector<std::vector<int>>& board) {
  ...
  if(std::any_of(board.begin(), board.end(),
    [](std::vector<int>& v) { return std::all_of(v.begin(), v.end(),
...
int boardSum(std::vector<std::vector<int>>& board) {
  return std::accumulate(board.cbegin(), board.cend(), 0,
    [](const int acc, const std::vector<int>& v) {
      return acc + std::accumulate(v.cbegin(), v.cend(), 0, 
          [](const int acc, const int i) { return acc + (i == -1? 0 : i); });
    });
}
  ...
  for(const int num : drawList) {
    for(std::vector<std::vector<int>>& board : boards) {
      ...
      if(checkBoardWon(board)) {
        print2dVector(board, "Winning Board");
        std::cout << boardSum(board)*num << std::endl;
        return 0;
      }
    }
  }
